...
class FerramentaAnalisadoraImagem(BaseTool):
    name:str = 'ferramenta_analisadora_imagem'
    description:str ='''
    Utilize esta ferramenta sempre que for solicitado que você faça uma análise de imagem.

    # Entradas Requiridas
    - 'nome_imagem' (str): O nome da imagem a ser analisado com extensão de JPG ou PNG.
    Exemplo: teste.jpg, teste.png, teste.jpeg
    '''
    ...
    def _run(self, acao):
        ...
        template_analisador = ChatPromptTemplate.from_messages(
            [
                (
                    'system',
                    '''
                    Assuma que você é um analisador de imagens. A sua tarefa principal consiste em:
                    analisar uma imagem e extrair informações importantes de forma objetiva.

                    # FORMATO DE SAÍDA
                    Descrição da imagem: 'Coloque a sua descrição da imagem aqui'
                    Rótulos: 'Coloque uma lista com três termos chaves separados por virgula'
                    '''
                ),
                (
                    'user',
                    [
                        ...
                        {
                            'type': 'image_url',
                            'image_url': {'url':'data:image/png;base64,{imagem_informada}'}
        ...
        template_resposta = PromptTemplate(
            ...
            A ideia é que a comunicação do resultado seja o mais facil possível, priorizando registros para consultas
            posteriores.
